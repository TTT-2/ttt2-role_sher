...
function ROLE:Initialize()
	...
	if CLIENT then
		...
		LANG.AddToLanguage("Italian", self.name, "Sceriffo")
		LANG.AddToLanguage("Italian", "info_popup_" .. self.name, [[Sei lo Sceriffo! Prova a prendere un Vice per proteggere gli Innocenti.]])
		LANG.AddToLanguage("Italian", "body_found_" .. self.abbr, "Era uno Sceriffo!")
		LANG.AddToLanguage("Italian", "search_role_" .. self.abbr, "Questa persona era uno Sceriffo!")
		LANG.AddToLanguage("Italian", "target_" .. self.name, "Sceriffo")
		LANG.AddToLanguage("Italian", "ttt2_desc_" .. self.name, [[Lo Sceriffo deve proteggere gli innocenti con il suo Vice! Se lo Sceriffo muore, anche il Vice lo farà (automaticamente).]])
		LANG.AddToLanguage("Italian", "credit_" .. self.abbr .. "_all", "Sceriffo, ti è stato dato {num} credito per la tua performance.")
